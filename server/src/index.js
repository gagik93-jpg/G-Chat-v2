...
const { clients, broadcastToChat } = initSocketManager(wss);
...
wss.on('connection', (ws, req) => {
  ws.on('message', async (message) => {
    try {
      ...
      switch (data.type) {
        case 'auth':
          if (!data.token) return ws.close(4001, "Unauthorized");
        ...
        case 'call_end':
          ...
          if (targetWs) {
            targetWs.forEach(tws => {
              if (tws.readyState === 1) tws.send(JSON.stringify({ type: data.type, ...data }));
            });
          }
          break;
      ...
      ws.send(JSON.stringify({ type: 'error', message: 'Invalid token or internal error' }));
    }
  });
